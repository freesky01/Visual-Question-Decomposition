...
class MiniGPTBase(BaseModel):
    ...
    def binary_loss(self, samples, reduction='mean'):
        samples_copy = copy.deepcopy(samples)
        for i in range(len(samples['conv_q'])):
            conv_q = samples['conv_q'][i]
            conv_a = samples['conv_a'][i]
            connect_sym = samples['connect_sym'][i]
            samples_copy['conv_q'][i] = conv_q.split(connect_sym)[0]
            samples_copy['conv_a'][i] = conv_a.split(connect_sym)[0]
        
        yes_no_labels = []
        for j in range(len(samples_copy['conv_q'])):
            if 'Yes' in samples_copy['conv_a'][j]:
                yes_no_labels.append([1.0, 0.0])
            else:
                yes_no_labels.append([0.0, 1.0])
        yes_no_labels = torch.FloatTensor(yes_no_labels).to(self.device)
        
        cond_embeds, cond_atts, regress_embeds, regress_atts, part_targets = \
            self.preparing_embedding(samples_copy)
        ...
        option_ids = [self.llama_tokenizer.encode(option, add_special_tokens=False)[0] for option in ['Yes', 'No']]
        ...
        binary_loss = loss_fct(pred, yes_no_labels)
        
        return binary_loss
        
            
    def forward(self, samples, zero_phase=False, reduction='mean'):
        binary_loss = self.binary_loss(samples)
