...
def parse_args():
    ...
    parser.add_argument("--gpt_eval_path", required=True, help="path to prediction output file.")
    parser.add_argument("--eval_output_path", required=True, help="path to evaluation output file.")
    args = parser.parse_args()
    return args


def contains_three_or_more_uppercase_letters(text):
    uppercase_letters = re.findall(r'[A-D]', text)
    return len(set(uppercase_letters)) >= 3


def tokenize_text_length(text):
    tokens = word_tokenize(text)
    return len(tokens)


def postprocess(data):
    for i, response_dict in enumerate(data):
        content = response_dict['choices'][0]['message']['content']
        subquestions = content.split('\n')
        if (contains_three_or_more_uppercase_letters(content) or
            tokenize_text_length(content) < 5 or
            len(subquestions) > 7):
            # Check if the 'choices' string exists
            if 'choices' in response_dict and len(response_dict['choices']) > 0:
                data[i]['choices'][0]['message']['content'] = "E"
            else:
                print(f"Skipping sample {response_dict["id"]} due to missing 'choices' key")
    return data
